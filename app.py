...
if __name__ == '__main__':
    ...
    if config.transaction and config.transaction.enabled:
        private_key = os.getenv("PRIVATE_KEY") or load_private_key(config.transaction.private_key_path)
        if private_key:
            transaction_interval = nm.interval_to_seconds(config.transaction.interval)
            start_transaction_task(web3, private_key, transaction_interval)
        else:
            logger.error("Transaction is enabled, but the private key could not be loaded. Transactions will not be sent.")
